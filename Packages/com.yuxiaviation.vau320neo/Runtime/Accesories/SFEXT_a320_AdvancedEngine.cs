    ...
    public class SFEXT_a320_AdvancedEngine : UdonSharpBehaviour
    {
        ...
        private void Power_OwnerUpdate(float deltaTime)
        {
            ...
            if (reversing)
            {
                ...
                throttleInput = (airVehicle.ThrottleOverridden > 0 && Input.GetAxis(gripAxis) < 0.75f && airVehicle.ThrottleInput > idlePoint + 0.02f) ?
                    0f
            ...
            //启动逻辑：N2到达16%时，N1才启转
            var targetN1 = (!isN2Runnning) ? Lerp3(0, 0, idleN1, n2, 0, (0.16f * takeOffN2), idleN2) : Lerp3(0, idleN1, takeOffN1, n2, 0, idleN2, takeOffN2);
            ...
            n1 = Mathf.MoveTowards(n1, targetN1, deltaTime * ((n1 < idleN1) ? n1StartupResponse : n1Response) * continuousN1 * Randomize());

            normalizedThrust = Mathf.Clamp01(Mathf.Pow(n1 / takeOffN1, thrustCurve));

            float minEGT = Mathf.Lerp(externalTempurature, compressorEGT, ClampedRemap01(n2, 0, minN2));
            var egtTarget = fire ? fireEGT : Lerp4(minEGT , fuel ? idleEGT : minEGT, continuousEGT, takeOffEGT, n2, 0, idleN2, continuousN2, takeOffN2);
            egt = Mathf.Lerp(egt, egtTarget, deltaTime * egtResponse * Randomize());

            var ffTarget = (starter ? 
                (n2 > startUpFFN2[0]? Lerp4(startUpFF[0], startUpFF[1], startUpFF[2], startUpFF[3], n2, startUpFFN2[0], startUpFFN2[1], startUpFFN2[2], startUpFFN2[3])
                :0)
        ...
        private void Power_Update(float deltaTime)
        {
            ...
            reverserPosition = TowWayMoveTowards(reverserPosition, reversing ? 0.99f : 0, deltaTime, reverserExtractResponse, reverserRetractResponse);
            if (vehicleAnimator)
            {
