        ...
        public void ToggleGear() {
            if (AllowToggleGrounded || !(bool)SAVControl.GetProgramVariable("Taxiing")) {
                if (!GearUp) {
                    ...
                    if (AllowToggleGrounded || !Physics.Raycast(CenterOfMass.position, -EntityControl.transform.up, GearCheckDistance, 133121/* Default, Environment, and Walkthrough */)) { SendCustomNetworkEvent(VRC.Udon.Common.Interfaces.NetworkEventTarget.All, nameof(SetGearDown)); }
                }
            }
        }
