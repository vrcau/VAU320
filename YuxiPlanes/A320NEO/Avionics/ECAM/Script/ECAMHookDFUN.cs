...
namespace YuxiFlightInstruments.ECAM {
    ...
    public class ECAMHookDFUN : UdonSharpBehaviour {
        ...
        public void SFEXT_O_OnPlayerJoined() {
            if (HookDown) { SendCustomNetworkEvent(VRC.Udon.Common.Interfaces.NetworkEventTarget.All, nameof(SetHookDown)); }
        }
        private void Update() {
            if (func_active) {
                float Trigger;
                if (UseLeftTrigger) { Trigger = Input.GetAxisRaw("Oculus_CrossPlatform_PrimaryIndexTrigger"); } else { Trigger = Input.GetAxisRaw("Oculus_CrossPlatform_SecondaryIndexTrigger"); }
            ...
            if (Hooked && (bool)SAVControl.GetProgramVariable("Taxiing")) {
                ...
                if (Vector3.Distance(VehicleTransform.position, HookedLoc) > HookedCableSnapDistance)//real planes take around 80-90 meters to stop on a carrier
                {
                    ...
                    if (HookedDelta < 5) { SendCustomNetworkEvent(VRC.Udon.Common.Interfaces.NetworkEventTarget.All, nameof(PlayCableSnap)); }
                }
        ...
        public void SetHookDown() {
            HookDown = true;
            ECAMController.IsHookDown = true;
        ...
        public void SetHookUp() {
            HookDown = false;
            ECAMController.IsHookDown = false;
